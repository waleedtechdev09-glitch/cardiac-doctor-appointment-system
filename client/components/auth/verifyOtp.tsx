...
const VerifyOtpForm = () => {
  ...
  const handlePaste = (event: React.ClipboardEvent<HTMLDivElement>) => {
    ...
    const nextOtp = Array.from({ length: OTP_LENGTH }, (_, index) => pastedValue[index] ?? "");
    setOtp(nextOtp);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!email) {
      setError("We need the email from the login step before verifying the code.");
      return;
    }
      ...
      <div className="grid min-h-[720px] w-full max-w-[1200px] grid-cols-1 overflow-hidden border border-slate-100 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.1)] md:grid-cols-2 md:rounded-3xl">
        <form className="flex w-full flex-col justify-center p-10 md:p-14 lg:p-20" onSubmit={handleSubmit}>
          <CardHeader className="mb-10 space-y-3 p-0">
            ...
            >
              <ShieldCheck className={`${sizing.iconSize} ${colors.textPrimary}`} />
            </div>
          ...
          <CardContent className="space-y-8 p-0">
            <div className="space-y-4">
              ...
              <div className="flex justify-center gap-2.5 md:justify-start md:gap-3" onPaste={handlePaste}>
                {otp.map((digit, index) => (
                  <Input
                    key={index}
                    ref={(element) => {
